<?php if ( !defined('ABS_PATH') ) exit('ABS_PATH is not loaded. Direct access is not allowed.');
    ...
    class Item extends DAO
    {
         ...
         */
		 ...
		 public function osc_model_list_recent_ads() {
			 
			$this->dao->select('*');
            $this->dao->from('`oc_t_item` i');
			$this->dao->join('oc_t_item_description d', ' i.pk_i_id = d.fk_i_item_id', 'LEFT');
			$this->dao->join('oc_t_item_location l', ' i.pk_i_id = l.fk_i_item_id', 'LEFT');
		...
		public function post_title_prefix($id) {
			$this->dao->select('*');
			$this->dao->from('`oc_t_item_meta` i');
			$this->dao->where('i.fk_i_item_id ='.$id);
			...
			$items = strtolower($items[0]['s_value']);
			
			if (strpos($items, 'buy') !== false) {
				$items = '[WTB]';
			} else if (strpos($items, 'sell') !== false) {
				$items = '[WTS]';
			} else if (strpos($items, 'ask') !== false) {
				$items = '[WTA]';
			} else if (strpos($items, 'offer') !== false) {
				$items = '[WTO]';
			} else if (strpos($items, 'rent') !== false) {
				$items = '[WTR]';	
			} else if (strpos($items, 'promote') !== false) {
				$items = '[WTP]';		
			} else {
				$items = '';
			}
		...
		public function exclude_price_cat($id1, $id2, $id3, $id4) {
			
			if($id2) {
				$sql_id2 = ' or c.fk_i_parent_id='.$id2;
			}  else {
				$sql_id2 = '';
			}
			
			if($id3) {
				$sql_id3 = ' or c.fk_i_parent_id='.$id3;
			}  else {
				$sql_id3 = '';
			}
			
			if($id4) {
				$sql_id4 = ' or c.fk_i_parent_id='.$id4;
			}  else {
				$sql_id4 = '';
			}
			
			$this->dao->select('*');
            $this->dao->from('`oc_t_category` c');
			$this->dao->where('c.fk_i_parent_id ='.$id1. $sql_id2 . $sql_id3 . $sql_id4);
