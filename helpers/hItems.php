<?php
    ...
    */
	...
	function price_status($cat_id) {
		
		$exclude_price_cat = Item::newInstance()->exclude_price_cat('8', '7', '105', '118'); // exclude cat ID

		foreach ($exclude_price_cat as $exclude_cat) {
			
			if($exclude_cat['pk_i_id'] == $cat_id) {
				// hide price
				$result = false;
				break;
			} else {
	...
	function osc_list_recent_ads() {
		
		return Item::newInstance()->osc_model_list_recent_ads();
		
    }
	
	function osc_title_prefix($item_id) {
		return Item::newInstance()->post_title_prefix($item_id);
	}
    ...
    function osc_format_price($price, $symbol = null) {
        if ($price === null) return osc_apply_filter ('item_price_null', __('Call For Price') );
