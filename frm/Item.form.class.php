<?php if ( ! defined('ABS_PATH')) exit('ABS_PATH is not loaded. Direct access is not allowed.');
    ...
    class ItemForm extends Form {
        ...
        static public function category_select($categories = null, $item = null, $default_item = null, $parent_selectable = false, $cur_cat_id = null)
        {
            ...
            foreach($categories as $c) {
                if ( !osc_selectable_parent_categories() && !$parent_selectable ) {
                    ...
                    if(isset($c['categories']) && is_array($c['categories'])) {
                        ...
                        ItemForm::subcategory_select($c['categories'], $item, $default_item, 1, $cur_cat_id);
                    }					
                } else {
        ...
        static public function subcategory_select($categories, $item, $default_item = null, $deep = 0, $cur_cat_id = null)
        {
            // Did user select a specific category to post in?
            $catId = Params::getParam('catId');
            if(Session::newInstance()->_getForm('catId') != ""){
            ...
            foreach($categories as $c) {
				...
				if($cur_cat_id > 0) {
                	$selected = ( (isset($cur_cat_id) && $cur_cat_id == $c['pk_i_id']) );
				} else {
                	$selected = ( (isset($item["fk_i_category_id"]) && $item["fk_i_category_id"] == $c['pk_i_id']) || (isset($catId) && $catId == $c['pk_i_id']) );
				}
        ...
        $("form[name=item]").validate({
            ...
            rules: {
                ...
                contactEmail: {
                    required: function(element) {
						if($("#contactEmail").hasClass('valid') || $("#contactPhone").hasClass('valid')) {
							return false;
						} else {
							return true;
						}
				...
				contactPhone: {
                    required: function(element) {
						if($("#contactPhone").hasClass('valid') || $("#contactEmail").hasClass('valid')) {
							return false;
						} else {
							return true;
						}
            ...
            messages: {
                ...
                contactName: {
                    ...
                    maxlength: "<?php echo osc_esc_js(__("Name: no more than 50 characters")); ?>."
                },
                contactEmail: {
                ...
                address: {
                    ...
                    maxlength: "<?php echo osc_esc_js(__("Address: no more than 200 characters")); ?>."
                }
            },
...
<script type="text/javascript">
	...
	?>
	 ...
	 $(document).ready(function(){
		var defaultLabel = $('#catId :selected').parent().attr('label');
		
		if(defaultLabel == 'Personals') {
    ...
    $("#catId").change(function(){
		...
		// This will return OptionGroup label eg: Personals
		<?php if($_GET['action'] != 'item_edit') { ?>
        ...
        if(cat_id != '') {
			...
			<?php if($_GET['action'] != 'item_edit') { ?>
			if(catPriceEnabled[cat_id] == 1 && label != 'Personals') {
                ...
                $(document).ready(function() {
                    ...
                    function canContinue(numUpload) {
                        ...
                        if(total<= <?php echo $maxImages;?>) {
                            ...
                            json.success = false;
                            $('#restricted-fine-uploader .qq-uploader').after($('<div class="alert alert-error"><?php echo osc_esc_js(sprintf(__('Too many items'), $maxImages)); ?></div>'));
                        }
                        return json;
                    }
